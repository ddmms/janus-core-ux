...
class EnvironmentsTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.current_installer: PackageInstaller | None = None
        self._setup_ui()
    ...
    def _setup_ui(self):
        ...
        self.table_envs.setHorizontalHeaderLabels(
            ["Name", "Python", "Models", "Default"]
        )
        ...
        self.btn_scan.setToolTip(
            "Auto-detect micromamba, conda, and virtual environments"
        )
        ...
        self.btn_add.setToolTip(
            "Browse for a python executable in a custom environment"
        )
        ...
        self.btn_install_janus.setStyleSheet(
            "background-color: #313244; padding: 4px 10px;"
        )
        self.btn_install_janus.clicked.connect(
            lambda: self.install_model_package("janus-core")
        )
        ...
        self.table_models.setHorizontalHeaderLabels(
            ["Model / Architecture", "PyPI Package", "Status", "Action"]
        )
        ...
        self.btn_install_custom.setStyleSheet(
    ...
    def populate_environments_table(self):
        ...
        for row, env in enumerate(envs):
            ...
            # Python version
            self.table_envs.setItem(
                row, 1, QTableWidgetItem(env.python_version or "Unknown")
            )

            # Supported models
            models_str = (
                ", ".join(env.supported_architectures)
                if env.supported_architectures
                else "None"
            )
    ...
    def _on_env_selected(self):
        ...
        for row, (_key, meta) in enumerate(MODEL_PACKAGE_MAP.items()):
            ...
            # Installed status
            is_installed = any(
                arch in env.supported_architectures for arch in meta["architectures"]
            )
            ...
            btn_action.clicked.connect(
                lambda checked=False, p=pkg_name: self.install_model_package(p)
            )
    ...
    def scan_environments(self):
        ...
        self.log_console.append_log(
            "[INFO] Scanning system for Python and micromamba environments..."
        )
        ...
        self.log_console.append_log(
            f"[SUCCESS] Discovered {len(self.env_mgr.environments)} environments."
        )
    ...
    def add_custom_env(self):
        ...
        if filepath and os.path.exists(filepath):
            ...
            self.environments_updated.emit()
            self.log_console.append_log(
                f"[SUCCESS] Added custom environment: {name} ({filepath})"
            )
    ...
    def set_selected_as_default(self):
        ...
        if env_name:
            ...
            self.environments_updated.emit()
            self.log_console.append_log(
                f"[SUCCESS] Set {env_name} as default environment."
            )
    ...
    def remove_selected_env(self):
        ...
        if env_name:
            reply = QMessageBox.question(
                self,
                "Remove Environment",
                f"Remove environment configuration for '{env_name}'?",
    ...
    def install_model_package(self, package_name: str):
        ...
        if not env_name:
            QMessageBox.warning(
                self,
                "No Selection",
                "Please select an environment from the table first.",
            )
            return
        ...
        self.log_console.append_log(
            f"[INFO] Starting uv pip install for {package_name} into {env.name}..."
        )
        self.current_installer = PackageInstaller(
            env.python_path, [package_name], parent=self
        )
        self.current_installer.log_line.connect(self.log_console.append_log)
        self.current_installer.finished_install.connect(
            lambda ok, msg: self._on_install_completed(env.python_path, ok, msg)
        )
